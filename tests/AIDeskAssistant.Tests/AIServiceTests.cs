...
public sealed class AIServiceTests
{
    ...
    public void BuildSystemPrompt_UsesInternalScreenshotWorkflow()
    {
        ...
        Assert.Contains("do not send a second prose planning message", prompt);
        ...
        Assert.Contains("Do not send double-escaped sequences like '\\\\n', '\\\\r', or '\\\\t'", prompt);
        ...
        Assert.Contains("close it or move focus back into the document body", prompt);
        Assert.Contains("Do not extend a Word selection blindly with repeated shift+arrow presses", prompt);
        Assert.Contains("partial text, duplicated text, or a corrupted word", prompt);
